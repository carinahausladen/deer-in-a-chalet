...
alpha2 =  2.0
alpha3 =  2.0
alpha1_values = np.linspace(2, 0.1, 40)  # change these values as desired
...
for i, alpha1 in enumerate(alpha1_values):
    ...
    ax.set_title(
        r"Dirichlet PDF on the 2-simplex: $\alpha_1={:.2f}, \alpha_2={:.2f}, \alpha_3={:.2f}$".format(alpha1, alpha2,
                                                                                                      alpha3),
        fontsize=14)
    ax.set_xlabel('x')
    ax.set_ylabel('y')
...
gif_filename = "plots/dirichlet_animation_asymmetric.gif"
